...
double transf_pic(double x)//g(x)
{
    return x*x-12 ;//sqrt(x+12)
}

double transf_pic2(double x)//g(x)
{
    return sqrt (x+12);
}
...
double bissection(double xa, double xb, int *i)//recebe o intervalo e apontador do contador (metodo intervalar)
{
    *i = 0;
    double xn, erro=5;
...
double falsa_pos(double a, double b, int *i)//recebe intervalo e apontador contador (metodo intervalar)
{
    double xn, erro=5.0;
    *i=0;
...
double picard_perno (double x0, int *i)
{
    *i=0;
    ...
    while (erro>= 0.00001)
    {
        if(fabs(dtransf_pic2(x0))>=1){// |g'(x)|<1 - garantia de convergência
            break;
            return -1;
        }
        gx0 = transf_pic2(x0);
...
/*
Metodos de paragem (Método do carda e Método da bisseção)

|x1 −x2| ≤ ε , critério de precisão absoluta
|(x1 −x2)/x1| ≤ ε ou |(x1 −x2)/x2| ≤ ε , critério de precisão relativa
| f (x1)− f (x2)| ≤ ε critério de anulação da função
n = N, critério do número de iterações

n ≤ nm +log2(b−a) (nm é o número de bits da mantissa)
n ≤ 3.3×nM  +log10(b−a)
*/

/*
Metodos de paragem (Método de Newton e Método de Picard Peano)

|x1 −x2| ≤ ε , critério de precisão absoluta
n = N, critério do número de iterações
NEWTON: df(x0)!=0
PICARDO PEANO: // |g'(x)|<1 - garantia de convergência

*/

//Ver vantagens e desvantagens de cada processo no pdf
